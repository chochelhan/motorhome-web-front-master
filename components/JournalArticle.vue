<template>
  <div v-on:scroll="$emit('scroll', $event)" v-bind:style="{'background-image': 'url(' + mainImageUrl + '?articlecover), url(' + mainImageUrl + '?5x5)'}" class="article">
    ...
    <div class="article-container">
      <profile-image v-on:click.native="showProfile(targetMemberInfo.creatorNo)" v-bind:src="targetMemberInfo.profileImageUrl" size="45px" class="article-writer-profile"></profile-image>
      <div class="article-writer">by {{targetMemberInfo.creatorName}}</div>
  ...
  methods: {
    ...
    load (viewed = false) {
      this.$api.get(`/journals/${this.id}`, {
        ...
        this.targetMemberInfo = response.data.targetMemberInfo
        ...
        this.$emit('load', {
          ...
          targetMemberInfo: this.targetMemberInfo
        })
        this.$nextTick(() => {
